...
"""
import argparse, json, os, re, subprocess, sys, time, urllib.request, ssl
...
def wrangler(args, env, timeout=300):
    r = subprocess.run(WRANGLER + args, env=env, capture_output=True, text=True, timeout=timeout)
...
def omgeving():
    ...
    if not env.get("CLOUDFLARE_API_TOKEN") or not env.get("CLOUDFLARE_ACCOUNT_ID"):
        raise SystemExit("CLOUDFLARE_API_TOKEN en CLOUDFLARE_ACCOUNT_ID ontbreken in de omgeving")
...
def deploy(map_klant, project="voorbeelden", slug=None, log=print):
    env = omgeving()
